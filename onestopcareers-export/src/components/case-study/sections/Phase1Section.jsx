...
function Step({ step, onNext, isLast }) {
  const [revealed, setRevealed] = useState(false);

  const handleSubmit = async (val) => {
    // behaviours tracked at parent level via prop callbacks if needed
  };
    ...
    <div className="mb-4">
      ...
      {!revealed ? (
        <ProduceFirst
          ...
          onSubmit={async () => { setRevealed(true); }}
        />
      ) : (
        <div className="space-y-2">
          ...
          <button onClick={onNext} className="w-full py-2.5 bg-phase1 text-white text-sm font-medium rounded-lg hover:bg-accent-dark transition-colors">
            {isLast ? 'See how your predictions compared →' : `Watch step ${step.num + 1} →`}
          </button>
        </div>
      )}
    </div>
...
export default function Phase1Section({ onDone, onMarkBehaviour }) {
    ...
    <div className="px-5 pb-6">
      ...
      {P1_STEPS.slice(0, currentStep + 1).map((step, i) => (
        <Step
          ...
          step={step}
          onNext={advance}
          isLast={currentStep === P1_STEPS.length - 1}
        />
      ))}
    </div>
  );
