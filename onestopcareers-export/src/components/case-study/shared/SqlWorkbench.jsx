...
export default function SqlWorkbench({
  id,
  filename = 'query.sql',
  dataKey = null,          // fixed key — null means free-form (auto-detect)
  placeholder = '-- Write your SQL query...\nSELECT ...',
  onQueryRun,              // callback(query) for parent to track query count
  showEvaluate = true,
}) {
    ...
    <div className="bg-sql-bg border border-sql-border rounded-xl overflow-hidden my-3">
      ...
      <div className="flex items-center justify-between px-3.5 py-2 bg-sql-surface border-b border-sql-border">
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-[10px] text-sql-kw font-semibold tracking-wide ml-1">{filename}</span>
        </div>
