...
// CP-6C + CP-6E: Sprint 6 — Color-Pulse Triggers + Priya Stakeholder Pings
//
// CHANGES FROM CP10/CP12:
//
//   [T1 — Color-Pulse Triggers]
//   - MILESTONE_PULSE_COLORS map: each milestone → accent hex.
//   - ArjunQueryTerminal now receives `pulseColor` prop = MILESTONE_PULSE_COLORS[index].
//   - Active milestone section header gets a matching glow border.
//   - M1 (Scope) Arjun opening explicitly orients user to the split-pane workstation.
//
//   [T5 — Priya Stakeholder Pings]
//   - usePriyaPing(milestoneIndex, messages, loading) custom hook.
//   - Fires once if user has been on the same milestone for > 5 minutes.
//   - Ping message from Priya references the live ₹-loss counter (calculated from
//     start time) and nudges the user back to action.
//   - Injected as { role: 'priya', ... } into the active milestone's chat feed
//     via an onPriyaPing callback.
//
//   All CP10 feed architecture, scroll-spy, milestone routing unchanged.
...
// ── Milestone → query terminal pulse color ────────────────────────────────────
// Triage/Scope = Orange, Funnel = Blue, Cohort = Purple
const MILESTONE_PULSE_COLORS = {
  0: ORANGE,  // Scope
  1: ORANGE,  // Dashboard KPI
  2: BLUE,    // Funnel
  3: PURPLE,  // Root Cause / Cohort
  4: GREEN,   // Impact
  5: RED,     // Respond
};

// ── Priya ping messages — triggered after 5min idle on a milestone ────────────
function buildPriyaPing(milestoneIndex, elapsedMs) {
  const elapsedMin  = Math.floor(elapsedMs / 60000);
  const lossRupees  = Math.round((elapsedMs / 1000) * 1200); // ₹1,200/sec burn rate
  ...
  const pings = [
    ...
    `Still here. ${lossDisplay} gone since incident started. Arjun, anything from your side? Need a number I can take upstairs.`,
    `Checking in — ${lossDisplay} total exposure now. Don't overthink this one. Even a directional answer is better than silence. Go.`,
    `${lossDisplay} and rising. I know this is complex but EOD is hard. What's the one thing you're most confident about right now?`,
  ];
...
// ── Priya ping hook ───────────────────────────────────────────────────────────
// Fires once per milestone if user has been idle for 5+ minutes.
// Returns { pingText, clearPing } — caller injects pingText into chat feed.
function usePriyaPing(milestoneIndex, hasFired, onPing) {
  const startTimeRef = useRef(Date.now());
  const firedRef     = useRef(false);

  // Reset timer whenever milestone changes
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const PING_DELAY_MS = 5 * 60 * 1000; // 5 minutes
    const t = setTimeout(() => {
      if (firedRef.current) return;
...
function CollapsedMilestoneCard({ milestone, index, conclusion, isExpanded, onToggle }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }} style={{ marginBottom: 8 }}>
      <div onClick={onToggle} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderRadius: isExpanded ? '10px 10px 0 0' : 10, cursor: 'pointer', background: `${color}06`, border: `1px solid ${color}18`, transition: 'background 0.2s, border-radius 0.2s' }}
      ...
      <AnimatePresence>
        {isExpanded && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }} style={{ overflow: 'hidden', padding: '12px 14px', background: `${color}04`, border: `1px solid ${color}18`, borderTop: 'none', borderRadius: '0 0 10px 10px' }}>
            <DataSnapshot />
            {!([1, 2, 3].includes(index)) && <p style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--ink3)', margin: 0 }}>No data visualization for this milestone.</p>}
          </motion.div>
        )}
      </AnimatePresence>
    </motion.div>
...
function LogUpdating() {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 0', marginBottom: 8 }}>
...
// [T1 CHANGE] M1 context message now explicitly orients user to the split-pane workstation
const M1_CONTEXT_MESSAGE = `Quick orientation before we start — this is the analytics war room.

You're looking at a split workstation:
• Left panel (this chat) — where you'll reason through each step with me. I'll ask questions, you'll answer. Think out loud.
• Right panel — where the data will appear. KPI dashboards, funnel tables, cohort matrices. Don't interpret them alone — bring your read back here.

Now, the situation: Priya said orders are down 8.3% week-over-week. Let's be precise about what that means:

• Orders = completed purchases. Not browsing, not adding to cart.
• WoW = this Tuesday vs last Tuesday. Priya's reference point.
• GMV ₹19L = the rupee value of those lost orders. And it's climbing.

Before I pull a single number — is this a North Bangalore problem, or could it be platform-wide and Priya just noticed it there first?`;

const M1_SCOPE_QUESTION = `Don't pull any data yet. Tell me your instinct — North Bangalore isolated, or platform-wide?`;

// ── Milestone 1: Scope ────────────────────────────────────────────────────────
function MilestoneScope({ onComplete, callArjunMilestone, onPriyaPing }) {
  ...
  // Priya ping — fires after 5min on this milestone
  usePriyaPing(0, false, useCallback((pingMsg) => {
  ...
  const send = useCallback(async () => {
    ...
    if (c) setConcept(c);
    if (advance) { setAdvanced(true); setTimeout(() => setShowSynthesis(true), 1000); }
    else setStage('input');
  ...
  return (
    <div>
      {['context','bridge','question','input','replied'].includes(stage) && (
        <motion.div key="context-msg" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', marginBottom: 20 }}>
      ...
      <AnimatePresence>
        {['bridge','question','input','replied'].includes(stage) && (
          <motion.div key="bridge" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '4px 0 20px 42px' }}>
            <div style={{ flex: 1, height: 1, background: `${ORANGE}20` }} />
            <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: ORANGE, opacity: 0.7, whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: 5 }}><ArrowDown size={10} /> Your turn first</span>
            <div style={{ flex: 1, height: 1, background: `${ORANGE}20` }} />
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {['question','input','replied'].includes(stage) && (
          <motion.div key="scope-q" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', marginBottom: 20 }}>
            <Avatar initials="AJ" color={ORANGE} size={32} />
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 6 }}>
                <span style={{ fontFamily: 'var(--mono)', fontSize: 11, fontWeight: 700, color: ORANGE }}>Arjun</span>
        ...
        {stage === 'input' && !advanced && (
          <motion.div key="input-area" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
              <motion.div animate={{ opacity: [1, 0.3, 1] }} transition={{ duration: 1.2, repeat: Infinity }} style={{ width: 6, height: 6, borderRadius: '50%', background: ORANGE, flexShrink: 0 }} />
              <span style={{ fontFamily: 'var(--mono)', fontSize: 10, fontWeight: 700, color: ORANGE, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Your turn — reply to Arjun</span>
            </div>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} style={{ padding: '8px 12px', borderRadius: 8, marginBottom: 8, background: `${BLUE}08`, border: `1px solid ${BLUE}16` }}>
              <p style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--ink2)', margin: 0, lineHeight: 1.6 }}>💡 Type what you think — Arjun will react to whatever you write.</p>
            </motion.div>
            <ChatInput value={input} onChange={setInput} onSend={send} loading={loading} placeholder="My instinct is this is probably isolated to North Bangalore because..." autoFocus />
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {showSynthesis && (
          <SynthesisPrompt prompt="Before we pull any data — write your problem statement in one sentence. What exactly are we investigating?" placeholder='We are investigating: [what dropped], in [where], compared to [when], because [why it matters]...' minChars={60} color={ORANGE} onSubmit={(sentence) => onComplete(sentence)} />
        )}
      </AnimatePresence>
      <div ref={bottomRef} />
    </div>
...
function MilestoneDashboard({ onComplete, callArjunMilestone, onPriyaPing }) {
  ...
  usePriyaPing(1, false, useCallback((pingMsg) => {
    setMessages(prev => [...prev, pingMsg]);
    onPriyaPing?.();
  }, [onPriyaPing]));
...
// ── Milestone 3: Funnel ───────────────────────────────────────────────────────
function MilestoneFunnel({ onComplete, callArjunMilestone, onPriyaPing }) {
  const [predictionDone, setPredictionDone] = useState(false);
  const [messages, setMessages]             = useState([]);
  const [input, setInput]                   = useState('');
  const [loading, setLoading]               = useState(false);
  ...
  usePriyaPing(2, false, useCallback((pingMsg) => {
    setMessages(prev => [...prev, pingMsg]);
    onPriyaPing?.();
  }, [onPriyaPing]));
      ...
      <AnimatePresence>
        {predictionDone && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
            ...
            {/* [T1] pulseColor=BLUE for funnel terminal — blue glow links to funnel chat messages */}
            ...
            <AnimatePresence>
              {showSynthesis && <SynthesisPrompt prompt="In one sentence — what did the funnel tell you that the dashboard couldn't?" placeholder="The funnel showed that the drop is happening at Add-to-Cart, specifically for new users, which means..." minChars={60} color={PURPLE} onSubmit={(sentence) => onComplete(sentence)} />}
            </AnimatePresence>
            <div ref={bottomRef} />
          </motion.div>
        )}
...
function MilestoneRootCause({ onComplete, callArjunMilestone, onPriyaPing }) {
  const [predictionDone, setPredictionDone] = useState(false);
  const [messages, setMessages]             = useState([]);
  const [input, setInput]                   = useState('');
  const [loading, setLoading]               = useState(false);
  ...
  usePriyaPing(3, false, useCallback((pingMsg) => {
    setMessages(prev => [...prev, pingMsg]);
    onPriyaPing?.();
  }, [onPriyaPing]));
  ...
  const send = useCallback(async () => {
    ...
    setMessages(prev => [...prev, { role: 'user', text: q, isNew: true }]);
    setLoading(true);
    const { text, advance, concept: c, expertAnalysis: ea } = await callArjunMilestone(q, 'rootcause');
    if (ea) setExpertAnalysis(ea);
    if (advance) { setRevealed(true); setTimeout(() => onComplete(null), 2500 + 1600); return; }
    setLoading(false);
    setMessages(prev => [...prev, { role: 'arjun', text, isNew: true }]);
    if (c) setConcept(c);
  }, [input, loading, callArjunMilestone, onComplete]);
      ...
      <AnimatePresence>
        {predictionDone && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
            ...
            {/* [T1] pulseColor=PURPLE for cohort terminal */}
            {arjunOpened && <ArjunQueryTerminal queryData={cohortQ} onComplete={() => setCohortReady(true)} pulseColor={PURPLE} />}
...
function MilestoneImpact({ onComplete, callArjunMilestone, onPriyaPing }) {
  const [messages, setMessages]           = useState([]);
  const [input, setInput]                 = useState('');
  const [loading, setLoading]             = useState(false);
  const [concept, setConcept]             = useState(null);
  ...
  usePriyaPing(4, false, useCallback((pingMsg) => {
    setMessages(prev => [...prev, pingMsg]);
    onPriyaPing?.();
  }, [onPriyaPing]));
  ...
  return (
    <div>
      ...
      <AnimatePresence>{concept && <ConceptChip concept={concept} onDismiss={() => setConcept(null)} />}</AnimatePresence>
      {arjunOpened && !showSynthesis && <ChatInput value={input} onChange={setInput} onSend={send} loading={loading} placeholder="e.g. 820 × ₹385 × 2.1 × 52 = ₹3.46Cr. At 65% recovery = ₹2.25Cr" />}
      <AnimatePresence>
        {showSynthesis && <SynthesisPrompt prompt="What's the one-line number you'd lead with in your memo to Priya?" placeholder="₹2.25Cr recoverable GMV at 65% recovery rate, if Biryani supply is restored within 3 weeks" minChars={40} color={PURPLE} onSubmit={(sentence) => onComplete(sentence)} />}
      </AnimatePresence>
      <div ref={bottomRef} />
    </div>
  );
  ...
  return (
    <div style={{ marginTop: 12 }}>
      ...
      <div style={{ borderRadius: 12, overflow: 'hidden', border: `1px solid ${focused ? ORANGE : canSend ? `${ORANGE}40` : 'rgba(255,255,255,0.09)'}`, boxShadow: focused ? `0 0 0 1px ${ORANGE}35` : 'none', transition: 'border-color 0.18s, box-shadow 0.18s' }}>
        <textarea value={value} onChange={e => onChange(e.target.value)} onFocus={() => setFocused(true)} onBlur={() => setFocused(false)} placeholder="Hey Priya — the drop is driven by..." rows={5}
...
function MilestoneRespond({ onComplete, callArjunMilestone, investigationLog, onPriyaPing }) {
  const [messages, setMessages]       = useState([]);
  ...
  return (
    <div>
      ...
      {investigationLog && investigationLog.filter(l => l.conclusion).length > 0 && (
        <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} style={{ marginBottom: 16, borderRadius: 10, border: '1px solid rgba(255,255,255,0.08)', overflow: 'hidden' }}>
          <button onClick={() => setLogExpanded(e => !e)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', background: 'rgba(255,255,255,0.03)', border: 'none', cursor: 'pointer', textAlign: 'left' }}>
            <span style={{ fontFamily: 'var(--mono)', fontSize: 10, fontWeight: 700, color: 'var(--ink3)', textTransform: 'uppercase', letterSpacing: '0.06em', flex: 1 }}>Your investigation trail — {investigationLog.filter(l => l.conclusion).length} findings</span>
...
export default function ArjunSocraticChat({ phase, onVizRequest, onAdvance, onMilestoneAdvance, onExpertAnalysesUpdate, onLogUpdate }) {
  ...
  // Priya ping handler — noop at feed level (each milestone handles its own)
  const handlePriyaPing = useCallback(() => {
    // Optionally: bump StrategyCase notification badge here in future
  }, []);
  ...
  return (
    <div>
      ...
      <MilestoneStrip currentIndex={activeMilestoneIndex} completedIndices={completedIndices} onMilestoneClick={handleMilestoneClick} />

      {/* Completed milestone cards */}
      {log.map((entry) => (
        <div key={entry.index} ref={el => { milestoneRefs.current[entry.index] = el; }}>
          <CollapsedMilestoneCard milestone={MILESTONES[entry.index]} index={entry.index} conclusion={entry.conclusion} isExpanded={expandedCards.has(entry.index)} onToggle={() => toggleCard(entry.index)} />
        </div>
      ))}

      <AnimatePresence>{updating && <LogUpdating />}</AnimatePresence>

      {/* Active milestone — glow border in milestone pulse color */}
      <AnimatePresence mode="popLayout">
        ...
        >
          ...
          {activeMilestoneIndex === 0 && <MilestoneScope onComplete={(c) => handleComplete(0, c)} callArjunMilestone={callArjunMilestone} onPriyaPing={handlePriyaPing} />}
          {activeMilestoneIndex === 1 && <MilestoneDashboard onComplete={(c) => handleComplete(1, c)} callArjunMilestone={callArjunMilestone} onPriyaPing={handlePriyaPing} />}
          {activeMilestoneIndex === 2 && <MilestoneFunnel onComplete={(c) => handleComplete(2, c)} callArjunMilestone={callArjunMilestone} onPriyaPing={handlePriyaPing} />}
          {activeMilestoneIndex === 3 && <MilestoneRootCause onComplete={(c) => handleComplete(3, c)} callArjunMilestone={callArjunMilestone} onPriyaPing={handlePriyaPing} />}
          {activeMilestoneIndex === 4 && <MilestoneImpact onComplete={(c) => handleComplete(4, c)} callArjunMilestone={callArjunMilestone} onPriyaPing={handlePriyaPing} />}
          {activeMilestoneIndex === 5 && <MilestoneRespond onComplete={(c) => handleComplete(5, c)} callArjunMilestone={callArjunMilestone} investigationLog={log} onPriyaPing={handlePriyaPing} />}
        </motion.div>
      </AnimatePresence>
