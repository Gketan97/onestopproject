...
// Phase 2 — Executive Memo component.
//
// Two modes:
//   1. WRITE mode: user composes a 4-sentence executive memo (120 char min).
//   2. EVAL mode: Arjun evaluates using EXEC_MEMO_RUBRIC dimensions
//      (Sizing 40%, Actionability 35%, Structure 25%).
//      Evaluation calls the Netlify /evaluate function with a structured rubric prompt.
//      Falls back to local scoring if the call fails.
//
// Props:
//   investigationLog   — array of { index, conclusion } from Phase 2
//   scenario           — P2_SCENARIO object
//   onComplete(score)  — called when user finalises; passes { sizing, actionability, structure, overall }

import React, { useState, useCallback, useRef, useEffect } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { ArrowRight, CheckCircle, AlertCircle, TrendingUp, Zap, AlignLeft } from 'lucide-react';
...
// ── Typewriter ────────────────────────────────────────────────────────────────
function useTypewriter(text, speed = 11, trigger = true) {
...
function ArjunLine({ text, isNew = true }) {
  const { displayed, done } = useTypewriter(text, 11, isNew);
  return (
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
        ...
        <span style={{ fontFamily: 'var(--mono)', fontSize: 10, fontWeight: 700, color, textTransform: 'uppercase', letterSpacing: '0.06em', flex: 1 }}>
          {label}
        </span>
      ...
      <p style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--ink3)', margin: 0, lineHeight: 1.6 }}>
        {description}
      </p>
      {feedback && (
        <p style={{ fontSize: 12, color: 'var(--ink2)', margin: '8px 0 0', lineHeight: 1.65, padding: '8px 10px', borderRadius: 8, background: `${barColor}08`, border: `1px solid ${barColor}15` }}>
          {feedback}
        </p>
      )}
    </motion.div>
...
// ── Local scoring fallback (used if API call fails) ───────────────────────────
function scoreLocally(memoText, rubric) {
  ...
  let feedback;
  if (overall >= 0.75) {
    feedback = rubric.feedback.high;
  } else if (sizing < 0.4) {
    feedback = rubric.feedback.medium_sizing;
  } else if (actionability < 0.4) {
    feedback = rubric.feedback.medium_action;
  } else if (structure < 0.4) {
    feedback = rubric.feedback.medium_structure;
  } else {
    feedback = rubric.feedback.low;
  }
...
You are now evaluating an executive memo written by the analyst. Respond ONLY with valid JSON, no preamble, no markdown backticks.
...
Analyst's investigation findings:
${logSummary}

Evaluate the memo on three dimensions. Return this exact JSON structure:
{
  "sizing": <0.0–1.0 float — does it include a specific number with recovery rate?>,
  "actionability": <0.0–1.0 float — does it specify WHO does WHAT by WHEN?>,
  "structure": <0.0–1.0 float — conclusion first, evidence second, impact third, action last?>,
  "sizingFeedback": "<one sentence of specific feedback on sizing>",
  "actionabilityFeedback": "<one sentence of specific feedback on actionability>",
  "structureFeedback": "<one sentence of specific feedback on structure>",
  "overallVerdict": "<2-3 sentences: what they did well, the one thing to fix, and whether this would hold up in a VP review>"
}`;
  ...
  try {
    const res = await fetch('/.netlify/functions/evaluate', {
      ...
      body: JSON.stringify({
        system: systemPrompt,
        messages: [{ role: 'user', content: `Evaluate this executive memo:\n\n"${memoText}"` }],
        max_tokens: 400,
      }),
    });
    ...
    return {
      ...local,
      sizingFeedback: null,
      actionabilityFeedback: null,
      structureFeedback: null,
      overallVerdict: local.feedback,
    };
  }
}
...
export default function ExecutiveMemo({ investigationLog, scenario = P2_SCENARIO, onComplete }) {
  const [mode, setMode]           = useState('intro'); // intro → write → evaluating → result
  ...
  useEffect(() => {
    // Delay arjun opening message slightly
    const t = setTimeout(() => setArjunOpening(true), 300);
    return () => clearTimeout(t);
  }, []);

  useEffect(() => {
    bottomRef.current?.scrollIntoView({ behavior: 'smooth', block: 'end' });
  }, [mode, scores, evalLoading]);
  ...
  // Build impactLine from investigation log
  const impactLine = investigationLog?.find(e => e.index === 4)?.conclusion
    || investigationLog?.find(e => e.index === 3)?.conclusion;
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        <span style={{ fontFamily: 'var(--mono)', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--ink3)' }}>
          Executive Memo — Final Synthesis
        </span>
      ...
      {investigationLog && investigationLog.filter(e => e.conclusion).length > 0 && (
        ...
        >
          <div style={{ padding: '8px 14px', background: 'rgba(255,255,255,0.02)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            <span style={{ fontFamily: 'var(--mono)', fontSize: 9, fontWeight: 700, color: 'var(--ink3)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
              Your investigation — use these in the memo
            </span>
          </div>
          <div style={{ padding: '10px 14px' }}>
            {investigationLog.filter(e => e.conclusion).map((entry, i) => {
              const colors = [ORANGE, BLUE, PURPLE, GREEN, RED, PURPLE];
              const c = colors[entry.index % colors.length];
              return (
                <div key={i} style={{
                  display: 'flex', gap: 10, padding: '5px 0',
                  borderBottom: i < investigationLog.filter(e => e.conclusion).length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none',
                }}>
                  <span style={{ fontFamily: 'var(--mono)', fontSize: 9, fontWeight: 700, color: c, flexShrink: 0, marginTop: 2, minWidth: 20 }}>
                    P{entry.index + 1}
                  </span>
        ...
        {(mode === 'write' || mode === 'evaluating') && (
          ...
          >
              ...
              {['① Root cause', '② Evidence', '③ Impact (₹)', '④ Action (who / when)'].map((step, i) => {
                ...
                return (
                  <span key={i} style={{
                    ...
                    color: cs[i], padding: '3px 9px', borderRadius: 6,
                    background: `${cs[i]}10`, border: `1px solid ${cs[i]}22`,
                  }}>
                    {step}
                  </span>
                );
            ...
            <div style={{
              ...
              border: `1px solid ${focused ? ORANGE : canSubmit ? `${ORANGE}45` : 'rgba(255,255,255,0.09)'}`,
              ...
              <div style={{
                ...
                <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: canSubmit ? GREEN : 'var(--ink3)' }}>
                  {mode === 'evaluating'
                    ? '⟳ Arjun is reading...'
                    : canSubmit
                    ? '✓ Ready — Arjun will evaluate this'
                    : `${charCount} / ${EXEC_MEMO_RUBRIC.minChars} chars min`}
                </span>
                <motion.button
                  onClick={handleSubmit}
                  disabled={!canSubmit || mode === 'evaluating'}
                  whileHover={canSubmit ? { scale: 1.04, y: -1 } : {}}
                  ...
                  style={{
                    ...
                    boxShadow: canSubmit ? `0 2px 14px ${ORANGE}45` : 'none',
                  }}
                >
                  {mode === 'evaluating'
                    ? <>
      ...
      {/* ── CTA to enter write mode (shown after intro) ── */}
      <AnimatePresence>
        ...
        {mode === 'result' && scores && (
          ...
          >
            ...
            <div style={{
              ...
              background: `${overallColor}08`, border: `1px solid ${overallColor}28`,
              marginBottom: 16,
            }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                <Avatar initials="AJ" color={ORANGE} size={32} />
                <div>
                  <p style={{ fontFamily: 'var(--mono)', fontSize: 9, color: ORANGE, opacity: 0.7, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 2 }}>
                    Arjun — Evaluation
                  </p>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                    <span style={{ fontFamily: 'var(--mono)', fontSize: 26, fontWeight: 900, color: overallColor, lineHeight: 1 }}>
                      {overallPct}
                    </span>
              ...
              <p style={{ fontSize: 14, lineHeight: 1.72, color: 'var(--ink)', margin: 0, fontWeight: 500 }}>
                {scores.overallVerdict || EXEC_MEMO_RUBRIC.feedback.high}
              </p>
            </div>
            ...
            <ScoreBar
              ...
              score={scores.sizing}
              color={PURPLE}
              description={EXEC_MEMO_RUBRIC.dimensions.sizing.description}
              feedback={scores.sizingFeedback}
            />
            <ScoreBar
              label="Actionability"
              icon={Zap}
              ...
              color={ORANGE}
              description={EXEC_MEMO_RUBRIC.dimensions.actionability.description}
              feedback={scores.actionabilityFeedback}
            />
            <ScoreBar
              label="Structure"
              icon={AlignLeft}
              ...
              color={BLUE}
              description={EXEC_MEMO_RUBRIC.dimensions.structure.description}
              feedback={scores.structureFeedback}
            />

            {/* Your submitted memo — reference */}
            <div style={{
              marginTop: 12, padding: '14px 16px', borderRadius: 12,
              background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)',
            }}>
              <p style={{ fontFamily: 'var(--mono)', fontSize: 9, fontWeight: 700, color: 'var(--ink3)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 8 }}>
                Your memo
              </p>
              <p style={{ fontSize: 13, lineHeight: 1.72, color: 'var(--ink2)', margin: 0, whiteSpace: 'pre-wrap', fontStyle: 'italic' }}>
                "{memo}"
              </p>
            </div>
            ...
            >
              ...
              <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: GREEN, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                Phase 2 Investigation Complete — saved to portfolio
              </span>
            </motion.div>
          </motion.div>
        )}
