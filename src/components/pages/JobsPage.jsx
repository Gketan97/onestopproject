...
const JobsPage = () => {
  ...
  const currentData = useMemo(
    () => (Array.isArray(filteredData) ? filteredData.slice(0, currentPage * ITEMS_PER_PAGE) : []),
    [currentPage, filteredData]
  );

  const hasMoreData = currentData.length < (filteredData?.length || 0);
  ...
  const renderContent = () => {
    ...
    if (!currentData?.length)
      return <p className="text-center text-gray-400 py-4 mt-8">No results found.</p>;

    if (activeTab === "jobs") {
      return (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
          {currentData.filter(Boolean).map((job) => (
            <JobCard
              key={job.id}
              job={job}
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
        {currentData.filter(Boolean).map((ref) => (
