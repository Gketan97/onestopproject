      ...
      <div className="jb">
        ...
        <div className="jb-header">
          ...
          <div className="jb-search-row">
            <div className="jb-search-wrap">
              <input className="jb-search" placeholder="Search by title or company…" value={q} onChange={e => setFilter('q', e.target.value)} />
              {q ? <button className="jb-search-clear" onClick={() => setFilter('q', '')}>×</button> : <span className="jb-search-icon">⌕</span>}
            </div>
          </div>
        ...
        <div className="jb-grid-wrap">
          ...
          {!loading && filtered.length > 0 && (
            <div className="jb-grid">
              {paginated.map(job => (
                <div key={job.id} className="jb-card" onClick={() => navigate(`/jobs/${job.id}${searchParams.toString() ? '?' + searchParams.toString() : ''}`)}>
                  <div className="jb-card-top">
                    ...
                    <span className="jb-card-posted" style={{ color: postedColor(job.posted_at) }}>{postedLabel(job.posted_at)}</span>
                  </div>
                  <div className="jb-card-bottom">
                    {job.fn && <span className="jb-tag cat">{CATEGORIES.find(c => c.fns.includes(job.fn))?.label || job.fn}</span>}
