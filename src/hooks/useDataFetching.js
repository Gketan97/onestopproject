...
export const useDataFetching = () => {
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      // Add a robust check to ensure data is a valid array before processing.
      ...
      // THE FIX: Stop renaming the keys. Pass the data through with its original structure.
      // This ensures consistency with what the rest of the application expects.
      const referralsWithIds = Array.isArray(referralsData)
        ? referralsData
            .filter(ref => ref && ref.Name) // This guard also prevents crashes
            .map((ref, index) => ({ ...ref, id: index })) // Pass original 'ref' object
        : []; 
