...
export default function SwiggyCase() {
  ...
  // Persist elapsed time periodically so refresh doesn't lose it (Bug 10)
  const handleTimerTick = useCallback((elapsedSeconds) => {
    update(prev => ({ ...prev, p2ElapsedSeconds: elapsedSeconds }));
  }, [update]);
      ...
      case 'phase1':
        return (
          <Phase1Section
            ...
            onSavePrediction={(stepIndex, val) => {
              update(prev => ({
                ...prev,
                p1Predictions: { ...prev.p1Predictions, [stepIndex]: val },
              }));
            }}
          />
        );
