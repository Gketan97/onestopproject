...
export default function DebriefSection({ state }) {
  const { behaviours, behaviourQuality, evidence, p2ElapsedSeconds, p2QueryCount, hints, completedPhases, p2Answers = {}, p3Answers = {}, candidateName = null } = state;
  ...
  // Bug 14 fix: snapshot stable values before effect to avoid stale-closure risk.
  // These are extracted at render time so the async callback always uses the
  // values that were current when the effect ran, not a stale previous render.
  useEffect(() => {
    let cancelled = false;
    // Snapshot at effect-run time (not closure-captured from outer scope)
    const snapAnswers = { ...p2Answers, ...p3Answers };
    const snapScore = score;
    const snapBehavioursDone = behavioursDone;
    const snapCandidateName = candidateName;
    const snapCompletedPhases = completedPhases;
    const snapBehaviours = behaviours;
    const snapP2QueryCount = p2QueryCount;
    const snapHints = hints;

    (async () => {
      const allAnswers = Object.entries(snapAnswers)
        .map(([k, v]) => `${k}: ${v}`).join('\n\n');
      const fb = await call(
      ...
      const keyQueries = Object.entries(snapAnswers)
      ...
      const scoreSummary = snapScore >= 75
        ? `${snapBehavioursDone}/8 analytical behaviours — strong performance.`
        : `${snapBehavioursDone}/8 behaviours demonstrated.`;

      await savePortfolioToService({
        portfolioId,
        candidateName:  snapCandidateName,
        caseStudyId:    'swiggy',
        caseStudyTitle: 'Swiggy Orders Investigation',
        phase2Queries:  p2Answers,
        phase3Answers:  p3Answers,
        finalWriteUp:   p2Answers['p2-vp-ta'] || '',
        aiEvaluation:   profileText + '\n\n' + interviewerText,
        score:          snapScore,
        scoreSummary,
        keyQueries,
        completedPhases: snapCompletedPhases,
        behaviours:      snapBehaviours,
        p2QueryCount:    snapP2QueryCount,
        hints:           snapHints,
      });
  ...
  }, []); // intentionally runs once on mount — values snapshotted inside
