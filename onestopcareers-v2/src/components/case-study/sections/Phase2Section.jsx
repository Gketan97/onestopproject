...
function AskArjunBar({ onAsk }) {
  ...
  const { callArjun: call } = useArjun();
    ...
    setLoading(true);
    setResponse('');
    const fb = await call(`Socratic question about Swiggy investigation: ${q}\nRespond Socratically — ask a question back, don't give the answer.`, 'clarify');
    setResponse(fb);
    setLoading(false);
    setQ('');
    onAsk?.();
  ...
  return (
    <div className="sticky bottom-0 border-t border-border bg-surface -mx-5 px-5 py-3 z-40">
      <div className="flex gap-2 items-center">
        <div className="w-7 h-7 rounded-lg bg-phase1-bg border border-phase1-border flex items-center justify-center font-mono text-[9px] font-bold text-phase1 flex-shrink-0">AJ</div>
        <input
          type="text"
          value={q}
        ...
        />
        <button onClick={ask} className="px-3 py-2 bg-phase2 text-white text-xs font-medium rounded-lg hover:bg-blue-700 transition-colors flex-shrink-0">Ask →</button>
      </div>
      {loading && (
...
function Step({ stepId, onDone, onBehaviour, queryCount, onQueryCount }) {
  ...
  const handlePFSubmit = useCallback(async ({ answer }) => {
    setShowNext(true);
  }, []);
    ...
    return (
      <div className="mb-4">
        ...
        <ProduceFirst
          ...
          onSubmit={({ answer }) => { onBehaviour('B7', 'Confirmed causation via temporal + geographic evidence for both causes'); setShowNext(true); }}
        />
        {showNext && (
    ...
    return (
      <div className="mb-4">
        ...
        <ProduceFirst
          ...
          onSubmit={({ answer }) => { onBehaviour('B8', 'Wrote VP message with S/C/R format, two owners, specific timelines'); onDone(answer); }}
        />
      </div>
    );
  }
...
export default function Phase2Section({ startTime, priya1Sent, priya2Sent, onPriyaMessage, onTick, onDone, onBehaviour, onQueryCount, queryCount, vpText }) {
  const [stepIdx, setStepIdx] = useState(0);
  const { fmtElapsed } = useP2Timer({ startTime, onPriyaMessage, onTick, priya1Sent, priya2Sent });
