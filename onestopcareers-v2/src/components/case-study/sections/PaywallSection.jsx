// Bug 13 fix: replaced document.getElementById imperative DOM mutation
// with React state (paying) driving button text and disabled prop.
    ...
    fetch('/.netlify/functions/create-order', {
      ...
      .then(order => {
        const opts = {
          ...
          theme: { color: '#C84B0C' },
          handler: () => { setPaying(false); onUnlock(); },
      ...
      .catch(() => {
        alert('Payment could not be initialised. Please try again.');
        setPaying(false);
      });
  };
    ...
    <div className="px-5 pb-6">
      ...
      <div className="bg-ink rounded-xl p-4">
        ...
        <button
          ...
          disabled={paying}
          className="w-full py-3.5 bg-accent text-white font-semibold rounded-xl text-[15px] hover:bg-accent-dark hover:-translate-y-px transition-all disabled:opacity-60 disabled:cursor-not-allowed disabled:translate-y-0"
        >
          {paying ? 'Opening payment…' : 'Unlock Phase 3 — ₹499 →'}
        </button>
        {IS_DEV && (
